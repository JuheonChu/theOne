...
<title>빽다방 상품 상세보기</title>
<link
	href="${pageContext.request.contextPath}/resources/css/paikdabang/itemdetail.css"
			...
			<div class="section goods-detail-top">
				<div id="thumbnail">
					<div class="wrapper-thumbnail-swiper">
						<div class="thumbnail-item" style="width: 600px;">
							<img src="${pageContext.request.contextPath}/resources/images/paikdabang/<%=product.getImage() %>" width="500" height="500"
								alt="상품 상세 이미지">
						</div>
					</div>
				</div>
				...
				<div class="goods-detail-info" id="goodsPurchase">
					<div class="detail-info-top">
						...
						<h1 class="name"><%=product.getMenu() %>( <%=product.getTemp() %> )</h1>
						<div class="price num">
					...
					%>
					<form action="${pageContext.request.contextPath}/product/dabang/addBasket?id=<%=member.getId() %>&pid=<%=product.getIdx() %>&franchisekey=<%=franchisekey %>&pid=<%=product.getIdx() %>" method="post">
					...
					<div>
						<a href="itemlist.jsp"><input type="submit" value="목록" class="btn btn-primary"
							style="width: 80px; font-size: 20px; margin-left: 390px; margin-top: 20px;"/></a>
					</div>
					<%} else { %>
					<form action="${pageContext.request.contextPath}/product/dabang/nullBuy?
